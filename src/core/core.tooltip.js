...
function mergeOpacity(colorString, opacity) {
	var colorVal = color(colorString);
...
function getTooltipSize(tooltip, model) {
	...
	each(model.title, maxLineWidth);

	// Body width
	ctx.font = fontString(bodyFontSize, model._bodyFontStyle, model._bodyFontFamily);
	each(model.beforeBody.concat(model.afterBody), maxLineWidth);
	...
	each(model.footer, maxLineWidth);
...
export default class Tooltip extends Element {
	...
	getBeforeBody() {
		...
		return isArray(lines) ? lines : lines !== undefined ? [lines] : [];
	}
	...
	update(changed) {
		...
		if (active.length) {
			...
			model.caretPadding = getValueOrDefault(tooltipPosition.padding, 2);
	...
	drawCaret(tooltipPoint, size, opacity) {
		var vm = this._view;
	...
	drawBody(pt, vm, ctx, opacity) {
		...
		// Before body lines
		each(vm.beforeBody, fillLineOfText);

		var drawColorBoxes = vm.displayColors;
		xLinePadding = drawColorBoxes ? (bodyFontSize + 2) : 0;

		// Draw body lines now
		each(body, function(bodyItem, i) {
			each(bodyItem.before, fillLineOfText);

			each(bodyItem.lines, function(line) {
		...
		// After body lines
		each(vm.afterBody, fillLineOfText);
	...
	drawFooter(pt, vm, ctx, opacity) {
		...
		if (footer.length) {
			...
			ctx.font = fontString(vm.footerFontSize, vm._footerFontStyle, vm._footerFontFamily);
	...
	handleEvent(e) {
		...
		changed = !arrayEquals(me._active, me._lastActive);
		me._lastActive = me._active;

		if (options.enabled || options.custom) {
			me._eventPosition = getRelativePosition(e, me._chart);
